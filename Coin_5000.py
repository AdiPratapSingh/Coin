...
class Blockchain:
    ...
    def fetch_longest_chain(self):
        ...
        for node in self.nodes:
            print((node+'full_chain'))
            curr_chain = requests.get(node+'/full_chain')
            if(curr_chain.status_code == 200 and curr_chain.json()['length']>max_length and self.check_validity(curr_chain.json()['chain'])):
                max_length = curr_chain['length']
                self.chain = curr_chain['chain']
                flag = True
        return flag
...
def mine_block():
    ...
    blockchain.mem_transaction(sender = node_address, receiver = 'http://127.0.0.1:5000', amount = 1)
...
def get_longest():
    ...
    if replaced:
        return jsonify({'message':'New Chain is : ',
                        'Chain':full_chain()[0]}), 200  # Might be some error
    else:
